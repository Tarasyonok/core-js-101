...
function isInsideCircle(circle, point) {
  const s = ((circle.center.x - point.x) ** 2 + (circle.center.y - point.y) ** 2) ** 0.5;
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  ans += String(Math.min(a, b));
  ans += ', ';
  ans += String(Math.max(a, b));
  if (isEndIncluded) {
    ans += ']';
  } else {
    ans += ')';
...
function isCreditCardNumber(ccn) {
  const list = String(ccn).split('');
  for (let i = list.length % 2; i < list.length; i += 2) {
    list[i] *= 2;
    if (list[i] > 9) {
      list[i] -= 9;
    }
...
function getCommonDirectoryPath(pathes) {
  const spl = pathes[0].split('/');
  let ans = '';
  for (let i = 0; i < spl.length; i += 1) {
    const curr = spl.slice(0, i).join('/').concat('/');
    let f = true;
    for (let j = 0; j < pathes.length; j += 1) {
      if (!pathes[j].startsWith(curr)) {
        f = false;
      }
    }
    if (f) {
...
function evaluateTicTacToePosition(position) {
  ...
  for (let i = 0; i < 3; i += 1) {
    line = [position[0][i], position[1][i], position[2][i]];
    if (line.join('') === 'XXX' || line.join('') === '000') {
      return line[0];
    }
  }
